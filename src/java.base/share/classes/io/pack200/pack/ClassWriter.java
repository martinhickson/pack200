...
import static io.pack200.pack.Constants.*;
...
import io.pack200.pack.ConstantPool.Entry;
import io.pack200.pack.ConstantPool.Index;
import io.pack200.pack.ConstantPool.MethodHandleEntry;
import io.pack200.pack.ConstantPool.NumberEntry;
import io.pack200.pack.Package.Class;
import io.pack200.pack.Package.InnerClass;
/**
...
class ClassWriter {
    ...
    private void reorderBSMandICS(Attribute.Holder h) {
        ...
        Attribute attributeSourceFileSpecial = h.getAttribute(Package.attrSourceFileSpecial);
        if (attributeSourceFileSpecial == null) return;
        if (icsAttr == null) return;

        int icsidx = h.attributes.indexOf(icsAttr);
        if (icsidx < (h.attributes.size()-1)) {
            h.attributes.remove(icsidx);
            h.attributes.add(icsAttr);
        }
        return;
    }
    
//    private void reorderBSMandICSOriginal(Attribute.Holder h) {
//        Attribute bsmAttr = h.getAttribute(Package.attrBootstrapMethodsEmpty);
//        Attribute icsAttr = h.getAttribute(Package.attrInnerClassesEmpty);
//        Attribute codeEmptyAttribute = h.getAttribute(Package.attrCodeEmpty);
//        Attribute attributeSourceFileSpecial = h.getAttribute(Package.attrSourceFileSpecial);
//        if (bsmAttr == null) return;
//        if (icsAttr == null) return;
//        
//
//        if (codeEmptyAttribute == null) return;
//       if ( attributeSourceFileSpecial  == null) return;
//
